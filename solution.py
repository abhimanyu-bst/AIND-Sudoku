...
def grid_values(grid):
    ...
    """
    grid_dict = {}
    for box, answer in zip(boxes, grid):
        if answer is '.':
            grid_dict[box] = '123456789'
        else:
            grid_dict[box] = answer
    return grid_dict
...
def only_choice(values):
    for key, u_lists in units.items():
...
def search(values):
    ...
    chosen_box = min(values, key=lambda i: len(values[i]) if len(values[i]) is not 1 else 10)
    if len(values[chosen_box]) == 1:
        #print('success')
        return values
